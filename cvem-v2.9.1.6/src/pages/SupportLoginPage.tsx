...
import { Eye, EyeOff, Headphones, ArrowRight, User, Lock, Zap } from 'lucide-react';
...
export default function SupportLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser(user);
      showToastMessage(`مرحباً ${user.name}!`, 'success');
      ...
      <div style={{ width: '100%', maxWidth: 460 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <Link to="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.85rem', textDecoration: 'none' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontWeight: 800, fontSize: '1.3rem', color: isLight ? '#0d3a6e' : '#e0f2fe', lineHeight: 1.2 }}>بوابة الدعم الفني</div>
        ...
        <div style={cardStyle}>
          ...
          <p style={{ textAlign: 'center', color: isLight ? '#6b7280' : 'rgba(224,242,254,0.55)', fontSize: '0.875rem', marginBottom: '1.75rem' }}>
            إدارة استفسارات العملاء والدعم الفني
          </p>
        ...
        <div style={{ marginTop: '1rem' }}>
          <Link to="/helpdesk/apply" style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.6rem',
            padding: '0.85rem 1.5rem', borderRadius: 14, width: '100%', boxSizing: 'border-box',
            background: isLight ? 'linear-gradient(135deg, #0070c8, #00B0FF)' : 'linear-gradient(135deg, #0d3a6e, #0070c8)',
            border: `1.5px solid ${isLight ? 'rgba(0,112,200,0.5)' : 'rgba(0,176,255,0.4)'}`,
            color: '#fff', fontWeight: 700, fontSize: '1rem',
            textDecoration: 'none',
            boxShadow: isLight ? '0 4px 16px rgba(0,112,200,0.25)' : '0 4px 16px rgba(0,176,255,0.2)',
          }}>
            <Headphones style={{ width: 20, height: 20 }} />
            <span>تقديم طلب توظيف في الدعم</span>
          </Link>
        </div>

        <div style={{ textAlign: 'center', marginTop: '0.75rem' }}>
          <Link to="/support" style={{ fontSize: '0.82rem', color: isLight ? '#6b7280' : 'rgba(224,242,254,0.45)', textDecoration: 'none' }}>
            عميل؟ <span style={{ color: isLight ? '#0070c8' : '#67e8f9', fontWeight: 600 }}>أرسل استفساراً هنا</span>
          </Link>
        </div>
